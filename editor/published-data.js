...
(function useCanonicalPublishedDataInEditor() {
  ...
  load = async function loadCanonicalEditorialData() {
    const [open, rejected, response] = await Promise.all([
    ...
    issues = [
      ...
      ...rejected.filter((item) => !item.pull_request),
      ...publishedEvents.map(issueFromPublishedEvent)
    ];
    const available = new Set(issues.map(laneOf).filter(Boolean));
    if (!available.has(activeLane)) activeLane = ["ready", "drafting", "new", "published", "trash"].find((lane) => available.has(lane)) || "new";
    render();
  };
