...
- A chat may not recite or lightly trim its article/source headline (including “I read [headline]”), use newsroom-process filler, or reuse a conversation skeleton with different speakers and nouns. Cross-file structural similarity is checked after article terms are masked. A generation failure remains in automated Drafting; there is no generic deterministic fallback that can enter Ready for Approval.
